...
```bash
# Store a memory
curl -X POST http://localhost:8720/sessions/{id}/observations \
  -H "Content-Type: application/json" \
  -d '{"tool_name":"user_note","content":"Prefer dark mode in all UIs"}'

# Recall relevant context
curl -X POST http://localhost:8720/context/resolve \
  -H "Content-Type: application/json" \
  -d '{"query":"UI preferences","user_id":"default"}'

# Search memories
curl -X POST http://localhost:8720/v1/memory:search \
  -H "Content-Type: application/json" \
...
```bash
# Via API
curl http://127.0.0.1:8720/facts?user_id=default
curl http://127.0.0.1:8720/system/status
